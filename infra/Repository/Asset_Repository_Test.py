...
def Update_Test(symbol_to_update, new_asset):
    ...
    if selection[0].Symbol == symb and\
        ...
        selection[0].Current_Price == cp and\
        selection[0].Category_Id == cid:
        print("Update_Test1 - PASSED")
    
    else:
        print("Update_Test1 - FAILED")

    repo.Update(symbol_to_update, new_price=1.1)

    # Selecting data
    selection = repo.Select(symbol_to_update)
    if selection[0].Current_Price == 1.1:
        print("Update_Test2 - PASSED")
    else:
        print("Update_Test2 - FAILED")

    repo.Update(symbol_to_update, new_category_id=2)

    # Selecting data
    selection = repo.Select(symbol_to_update)
    if selection[0].Category_Id == 2:
        print("Update_Test3 - PASSED")
    else:
        print("Update_Test3 - FAILED")
...
# Is not possible to test asset without a category
