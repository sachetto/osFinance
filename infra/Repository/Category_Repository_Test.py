...
def InsertAndSelect_Test(c1, c2):
    with DBConnectionHandler(path) as db_connection:
        ...
        if selection[0].Category == c1 and\
            ...
            print("InsertAndSelect_Test1 - PASSED")
        else:
            print("InsertAndSelect_Test1 - FAILED")

        if selection[0].Id == 1 and\
            selection[1].Id == 2:
            print("InsertAndSelect_Test2 - PASSED")
        else:
            print("InsertAndSelect_Test2 - FAILED")

        #Selecting specific data
        selection = repo.Select(c1)
        if selection.Category == c1:
            print("InsertAndSelect_Test3 - PASSED")
        else:
            print("InsertAndSelect_Test3 - FAILED")


def InsertUndefinedCategory_Test():
    with DBConnectionHandler(path) as db_connection:
        repo = CategoryRepository(db_connection)
        repo.InsertUndefinedCategory()
        test_pass = False
        ...
        for item in selection:
            if item.Category == 'UNDEFINED':
                test_pass = True
                print("InsertUndefined_Test - PASSED")
                break
                
        if not test_pass:
            print("InsertUndefined_Test - FAILED")

def SelectUndefinedCategoryID_Test():
    with DBConnectionHandler(path) as db_connection:
        undefined_category_id = CategoryRepository(db_connection)\
                .SelectUndefinedCategoryID()

        if undefined_category_id:
            print("SelectUndefinedCategory_Test - PASSED")
                
        else:
            print("SelectUndefinedCategory_Test - FAILED")
...
InsertUndefinedCategory_Test()
SelectUndefinedCategoryID_Test()
